...
def test_suggest_groups_filters_and_ranks() -> None:
    client = _DummyClient()

    groups = suggest_groups(client, card_type_contains="A100", min_free_cards=2, min_card_memory_gb=80)

    assert [group.group_name for group in groups] == ["public-a100"]


def test_suggest_images_prefers_public_and_filters() -> None:
    client = _DummyClient()

    images = suggest_images(client, image_type="pytorch", name_contains="pytorch", prefer_public=True, limit=2)
