...
class AsyncNodesAPI:
    ...
    async def list(
        ...
        *,
        ...
        nodes = [
            Node.from_api(item)
            for item in await self._c.list_all("/api/iresource/v1/node", params=params)
        ]
    ...
    async def get(self, node_id: str) -> Node:
        data = await self._c.get(f"/api/iresource/v1/node/{node_id}")
...
class AsyncGroupsAPI:
    ...
    async def list(self, *, refresh: bool = False) -> list[ResourceGroup]:
        return self._aggregate(await self._c.nodes.list(refresh=refresh))

    async def by_name(self, name: str) -> ResourceGroup | None:
        for group in await self.list():
            if group.group_name == name:
                return group
        return None

    async def resolve_many(self, query: str) -> list[ResourceGroup]:
    ...
    async def resolve(self, name_or_id: str) -> ResourceGroup:
        return _resolve_one(
            name_or_id,
            await self.list(),
            key_fns=(
    ...
    async def resolve_id(self, name_or_id: str) -> str:
        return (await self.resolve(name_or_id)).group_id
