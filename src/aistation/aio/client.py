...
from ..transport import auth_flow
from .images import AsyncImagesAPI
from .resources import AsyncGroupsAPI, AsyncNodesAPI
from .tasks import AsyncTasksAPI
from .transport import auth_flow as async_auth_flow
from .transport import runtime as async_runtime
from .transport import session as async_session
from .workplatform import AsyncWorkPlatformsAPI
...
class AsyncAiStationClient:
    """Async SDK entrypoint."""
    ...
    def __init__(
        ...
        *,
        ...
        self.session = async_session.build_async_session(verify=self.config.verify_ssl)
        self.user: User | None = None
        self._auth_lock: asyncio.Lock | None = None

        self.nodes = AsyncNodesAPI(self)
        self.groups = AsyncGroupsAPI(self)
        self.images = AsyncImagesAPI(self)
        self.tasks = AsyncTasksAPI(self)
        self.workplatforms = AsyncWorkPlatformsAPI(self)
    ...
    async def login(
        ...
        *,
        ...
        async with self._get_auth_lock():
            reused = self._reuse_fresh_auth_unlocked(
                account=account,
                password=password,
                captcha=captcha,
            )
            if reused is not None:
                return reused
            return await self._login_unlocked(account=account, password=password, captcha=captcha)
    ...
    async def prepare_auth(self) -> User | None:
    ...
    async def fetch_captcha(self) -> str:
        from ..transport.envelope import check_flag as _check_flag

        body = await self._raw_get("/api/ibase/v1/captcha")
    ...
    async def refresh_user(self) -> User:
        await self.prepare_auth()
        data = await self.get("/api/ibase/v1/user/base", timeout=10)
    ...
    async def ensure_auth(self) -> User:
        async with self._get_auth_lock():
            restored = auth_flow.ensure_auth(self.session, self.auth, self.user)
    ...
    async def require_user(self) -> User:
        if auth_flow.has_user_profile(self.user):
            assert self.user is not None
            self._set_token_header(self.user.token)
            return self.user

        restored = auth_flow.ensure_auth(self.session, self.auth, self.user)
        if auth_flow.has_user_profile(restored):
        ...
        if restored is not None:
            self.user = restored
            try:
                return await self.refresh_user()
            except AuthError:
                if self.auth.account and self.auth.password:
                    return await self._reauth_login()
                raise
    ...
    async def __aenter__(self) -> AsyncAiStationClient:
        if self._should_prepare_auth():
            await self.prepare_auth()
        return self
    ...
    async def post(
        ...
        *,
        ...
        return await self._request_with_retry("POST", path, json=json, timeout=timeout)
    ...
    async def put(
        ...
        *,
        ...
        return await self._request_with_retry("PUT", path, json=json, timeout=timeout)
    ...
    async def paginate(
        ...
        *,
        ...
        while True:
            ...
            data = await self.get(path, params=query, timeout=self.config.default_timeout)
            if not isinstance(data, dict):
                break
            rows = data.get("data")
    ...
    async def list_all(
        ...
        *,
        ...
        data = await self.get(path, params=query, timeout=timeout)
    ...
    async def _raw_request(
        ...
        *,
        ...
        return await async_runtime.raw_request(
            self.session,
            self.base_url,
            self.config,
            method,
    ...
    async def _raw_get(
        ...
        return await self._raw_request("GET", path, params=params, timeout=timeout)
    ...
    async def _raw_post(
        ...
        return await self._raw_request("POST", path, json=json, timeout=timeout)
    ...
    async def _raw_put(
        ...
        return await self._raw_request("PUT", path, json=json, timeout=timeout)

    async def _raw_delete(self, path: str, timeout: float | None = None) -> dict[str, Any]:
        return await self._raw_request("DELETE", path, timeout=timeout)
    ...
    async def _request_with_retry(
        ...
        *,
        ...
        if self._should_prepare_auth():
            await self.prepare_auth()
        return await async_runtime.request_with_retry(
    ...
    def _timeout_for(self, path: str) -> float:
        return async_runtime.timeout_for(self.config, path)
    ...
    async def _reauth_login(self) -> User:
    ...
    async def ping(self) -> dict[str, Any]:
        ...
        t0 = _time.time()
        try:
            body = await self._raw_get("/api/ibase/v1/system/secret", timeout=10)
        ...
        if self.auth.token:
            self._prime_token_header()
            try:
                body = await self._raw_get("/api/ibase/v1/system/identity-source/type", timeout=10)
    ...
    def _reuse_fresh_auth_unlocked(
        self,
        *,
        account: str | None = None,
        password: str | None = None,
    ...
    async def _login_unlocked(
        ...
        *,
        ...
        self.user = await async_auth_flow.login(
            base_url=self.base_url,
