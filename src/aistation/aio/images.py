...
class AsyncImagesAPI:
    ...
    async def list(
        ...
        *,
        ...
        if images is None:
            images = [
                Image.from_api(item)
                for item in await self._c.list_all("/api/iresource/v1/images/all")
            ]
    ...
    async def types(self) -> builtins.list[ImageType_]:
        cached = self._types_cache.get()
        if cached is not None:
            return cached
        data = await self._c.get("/api/iresource/v1/image-type")
    ...
    async def resolve_many(self, image_ref: str) -> list[Image]:
    ...
    async def resolve(self, image_ref: str) -> Image:
        return _resolve_one(
            image_ref,
            await self.list(),
            key_fns=(
    ...
    async def check(self, image_name: str, image_tag: str) -> OperationResult[Image]:
        payload = {"imageName": image_name, "imageTag": image_tag}
        data = await self._c.post(
    ...
    async def import_external(
        ...
        *,
        ...
        data = await self._c.post("/api/iresource/v1/images/outside-import", json=body)
        if not isinstance(data, dict):
    ...
    async def progress(self, task_id: str) -> dict[str, Any]:
        data = await self._c.get("/api/iresource/v1/images/progress", params={"id": task_id})
        if not isinstance(data, dict):
